...
@Service
public class HuaShuoRdoV2Service extends ChannelService{
	public final static String RES_SUCCESS = "200";  // 请求通道成功
	public final static String PAY_SUCCESS = "0000";  // 扣费成功
	private  Logger log = Logger.getLogger(HuaShuoRdoV2Service.class);		
	@Override
	public String getPipleId() {
		return "14673525835107348590762";
	}
	@Override
...
//		String ip = requestBody.optString("ip");
		...
		if(StringUtil.isEmptyString(productCode) || StringUtil.isEmptyString(apiKey)   || StringUtil.isEmpty(pipleId)){
			...
			try {
				...
				String reqUrl = piple.getPipleUrlA()+"?"+"channel="+piple.getPipleAuthA()+"&mobile="+order.getMobile()+"&param="+param+"&type=mobile";
				...
				if(StringUtil.isNotEmptyString(pipleResult)){
					...
					if("0000".equals(code)){
						...
						String pipleOrderId = data.optString("orderNo");
						...
						order.setModTime(DateTimeUtils.getCurrentTime());
						order.setResultCode(object.optString("resultCode"));
					}
				}else{
		...
		try {
			...
			if(tOrder==null){
				...
				if(bResult!=null){// 返回不为空则校验不通过
					...
					String reqUrl = piple.getPipleUrlB()+"?"+"channel="+piple.getPipleAuthA()+"&smscode="+verifyCode+"&param="+param+"&orderno="+tOrder.getPipleOrderId();
					...
					if(StringUtil.isNotEmptyString(resultStr)){
						...
						if("0000".equals(code)){
							...
							tOrder.setResultCode(object.optString("resultCode"));
							tOrder.setModTime(DateTimeUtils.getCurrentTime());
