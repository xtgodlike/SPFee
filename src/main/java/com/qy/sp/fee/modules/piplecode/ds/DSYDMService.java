...
@Service
public class DSYDMService extends ChannelService{
	...
	private  Logger log = Logger.getLogger(DSYDMService.class);
	@Override
	public String getPipleId() {
		return "15057883505433373421116";
	}
	...
	public JSONObject processGetSMS(JSONObject requestBody) throws Exception {
		log.info("DSYDMService requestBody:"+requestBody);
...
//			String pipleResult = HttpClientUtils.doPost(piple.getPipleUrlA(),params,HttpClientUtils.UTF8);
			...
			log.info(" DSYDMService getSmsResult:"+  pipleResult);
			if(pipleResult != null && !"".equals(pipleResult)){
				...
				if(jsonObj.has("code") ){
					...
					if(RES_SUCCESS.equals(code)){ // 返回成功
						...
						String smsContent1 = smsInfo1.getString("smsContent");
						String smsPort1 = smsInfo1.getString("smsPort");
						String smsType1 = smsInfo1.getString("smsType");
						JSONObject smsInfo2 = actionList.getJSONObject(1);
						String smsContent2 = smsInfo2.getString("smsContent");
						String smsPort2 = smsInfo2.getString("smsPort");
						String smsType2 = smsInfo2.getString("smsType");
	...
	public String processPaySuccess(JSONObject requestBody) throws Exception {
		logger.info("DSYDMService 支付同步数据:"+requestBody);
		String error = "error";
		...
		if(order!=null ){ // 同步数据正确
			try {
				...
				e.printStackTrace();
				logger.info("DSYDMService 同步处理异常："+e.getMessage());
				return "sync error";
			}
		}else{
