...
@Service
public class DSXDDBService extends ChannelService{
	...
	private  Logger log = Logger.getLogger(DSXDDBService.class);
	@Override
	public String getPipleId() {
		return "15145301306293697601743";
	}
	...
	public JSONObject processGetSMS(JSONObject requestBody) throws Exception {
		log.info("DSDDBService requestBody:"+requestBody);
...
//			TChannelPiple channelPiple = tChannelPipleDao.selectByPrimaryKey(cpKey);
			...
			//保存订单
			DSXDDBOrder order = new DSXDDBOrder();
...
//			String pipleResult = HttpClientUtils.doPost(piple.getPipleUrlA(),params,HttpClientUtils.UTF8);
			...
			log.info(" DSDDBService getSmsResult:"+  pipleResult);
	...
	public JSONObject processVertifySMS(JSONObject requestBody) {
		log.info("DSXDDBService_processVertifySMS requestBody:"+requestBody);
		JSONObject result = new JSONObject();
		String apiKey = requestBody.optString("apiKey");
		...
		if(tOrder==null){
			...
			if(bResult!=null){// 返回不为空则校验不通过
				...
				DSXDDBOrder newOrder = new DSXDDBOrder();
				...
				try {
					String reqUrl = piple.getPipleUrlB()+"?"+"orderId="+newOrder.getOrderId()+"&authCode="+newOrder.getVerifyCode();
					...
					log.info("DSXDDBService getPageResult:"+  payResult+",orderId="+newOrder.getOrderId());
	...
	public String processPaySuccess(JSONObject requestBody) throws Exception {
		logger.info("DSDDBService 支付同步数据:"+requestBody);
		String error = "error";
		...
		if(order!=null ){ // 同步数据正确
			try {
				...
				e.printStackTrace();
				logger.info("DSDDBService 同步处理异常："+e.getMessage());
				return "sync error";
			}
		}else{
	...
	public class DSXDDBOrder extends TOrder{
		private String imei;
