...
@Service
public class DSDMService extends ChannelService{
	...
	private  Logger log = Logger.getLogger(DSDMService.class);
	@Override
	public String getPipleId() {
		return "15028646126287883587215";
	}
	...
	public JSONObject processGetSMS(JSONObject requestBody) throws Exception {
		log.info("DSDMService requestBody:"+requestBody);
...
//			String pipleResult = HttpClientUtils.doPost(piple.getPipleUrlA(),params,HttpClientUtils.UTF8);
	...
	public String processPaySuccess(JSONObject requestBody) throws Exception {
		logger.info("DSDMService 支付同步数据:"+requestBody);
		String error = "error";
		...
		if(order!=null ){ // 同步数据正确
			try {
				...
				e.printStackTrace();
				logger.info("DSDMService 同步处理异常："+e.getMessage());
				return "sync error";
			}
		}else{
	...
	public class DSDMOrder extends TOrder{
		private String imei;
